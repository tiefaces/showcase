...
public class ItemSearchHelper {
	...
	private static String assembleWhereClause(String whereclause,
			String fieldclause, String value) {

		if ((value != null) && (!value.isEmpty())) {
			Object[] params = { value };
			String sql = new MessageFormat(fieldclause).format(params,
	...
	public static List<Object> dosearch(Object searchItem) {
		...
		String whereclause = "";
		whereclause = assembleWhereClause(whereclause,
				" upper(code) like upper(''%{0}%'')", item.getCode());
		whereclause = assembleWhereClause(whereclause,
				" upper(name) like upper(''%{0}%'')", item.getName());
		whereclause = assembleWhereClause(whereclause,
				" upper(description) like upper(''%{0}%'')",
				item.getDescription());
		if (item.getPrice() != null) {
			whereclause = assembleWhereClause(whereclause, " price = {0}", item
					.getPrice().toString());
		}
