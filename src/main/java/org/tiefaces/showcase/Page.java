...
public class Page {
	...
	private void loadIfNecessary() {
		...
		try {
			Map<String, Object> attributes = FacesUtility.getMetadataAttributes();
...
//			documentation = (apiPaths.size() + vdlPaths.size() > 0) ? new Documentation(apiPaths, vdlPaths) : null;
	...
	private static String loadDescription(List<String> apiPaths) {
		if (apiPaths.size() == 1) {
			...
			String url = String.format("%s%s.html", FacesUtility.evaluateExpressionGet("#{_apiURL}"), apiPaths.get(0));
	...
	private static String loadSourceCode(String path) {
		...
		try {
			return new String(FacesUtility.toByteArray(input), "UTF-8").replace("\t", "    "); // Tabs are in HTML <pre> presented as 8 spaces, which is too much.
		}
		catch (IOException e) {
