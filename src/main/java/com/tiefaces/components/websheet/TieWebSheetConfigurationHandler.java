...
public class TieWebSheetConfigurationHandler {
	...
	private Map<String, Integer> buildSchemaMap(int version) {
		...
		Map<String,Integer> schemaMap = new HashMap<String, Integer>(){{
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_TAB_NAME,0); 
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_SHEET_NAME,1); 
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_HEADER_RANGE,2);
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_BODY_RANGE,3);
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_FOOTER_RANGE,4);
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_BODY_TYPE,5);
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_ALLOW_ADD_ROW,6);
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_INIT_ROWS,7);
		       put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_PAGE_TYPE,8);
		}};
		...
		schemaMap.put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_ATTRIBUTE_VALUE,attributeStartColumn+2);
		schemaMap.put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_VALIDATION_ERROR_MSG,attributeStartColumn+3);

		schemaMap.put(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_VERSION,version);
	...
	public  Map<String, SheetConfiguration> buildConfiguration() {
		...
		Sheet sheet1 = parent.getWb().getSheet(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SHEET);
	...
	private  Map<String, SheetConfiguration> buildConfigurationWithTab(Sheet sheet1, Map<String, SheetConfiguration> sheetConfigMap) {
		...
		while (rowIterator.hasNext()) {
			...
			if (row.getRowNum() ==0) {
				...
				if (newTabName.isEmpty()) {
					addAttributesToMap(sheetConfig.getCellFormAttributes(), row, schemaMap);
				} else {
					...
					sheetConfig.setSheetName(rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_SHEET_NAME)));
					String tempStr = rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_HEADER_RANGE));
					sheetConfig.setFormHeaderRange(tempStr);
					sheetConfig.setHeaderCellRange(new CellRange(tempStr));
					tempStr = rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_BODY_RANGE));
					sheetConfig.setFormBodyRange(tempStr);
					sheetConfig.setBodyCellRange(new CellRange(tempStr));
					tempStr = rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_FOOTER_RANGE));
					...
					if (rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_FORM_BODY_TYPE)).equalsIgnoreCase("Repeat"))
...
debug("Web Form ConfigurationHandler after iteration sheetConfigmap = "+sheetConfigMap);					
	...
	private  Map<String, SheetConfiguration> buildConfigurationWithoutTab(Map<String, SheetConfiguration> sheetConfigMap) {
	    ...
	    for (int i = 0; i < parent.getWb().getNumberOfSheets(); i++) {
			...
			int rightCol = 0;
		    for (Row row : sheet) {
		    	if (row.getLastCellNum()>rightCol) rightCol = row.getLastCellNum(); 
		    }
			
			
			// header range row set to 0 while column set to first column to max column (FF) e.g. $A$0 : $FF$0
			String tempStr = "$"+TieWebSheetUtility.GetExcelColumnName(leftCol)+"$0 : $"+TieWebSheetUtility.GetExcelColumnName(rightCol)+"$0"; 
			sheetConfig.setFormHeaderRange(tempStr);
			sheetConfig.setHeaderCellRange(new CellRange(tempStr));
			// body range row set to first row to last row  while column set to first column to max column (FF) e.g. $A$1 : $FF$1000
			tempStr = "$"+TieWebSheetUtility.GetExcelColumnName(leftCol)+"$"+ (firstRow + 1) +" : $"+TieWebSheetUtility.GetExcelColumnName(rightCol)+"$"+(lastRow + 1); 
			sheetConfig.setFormBodyRange(tempStr);
			sheetConfig.setBodyCellRange(new CellRange(tempStr));
			sheetConfig.setFormBodyType(TIEConstants.TIE_WEBSHEET_FORM_TYPE_FREE);
			sheetConfig.setCellFormAttributes(new HashMap<String, List<CellFormAttributes>>());
	...
	private void addAttributesToMap(Map<String, List<CellFormAttributes>> map, Row row, Map<String, Integer> schemaMap) {
		List<CellFormAttributes> attributes =  map.get(rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_TARGET_COLUMN_CELL)));
		if (attributes == null) {
			map.put(rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_TARGET_COLUMN_CELL)), new ArrayList<CellFormAttributes>());
			attributes =  map.get(rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_TARGET_COLUMN_CELL)));
		}
		CellFormAttributes cellattribute = new CellFormAttributes();
		cellattribute.setType(rowCell(row, schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_ATTRIBUTE_TYPE)));
		cellattribute.setValue(rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_ATTRIBUTE_VALUE)));
		cellattribute.setMessage(rowCell(row,schemaMap.get(TIEConstants.TIE_WEBSHEET_CONFIGURATION_SCHEMA_VALIDATION_ERROR_MSG)));
